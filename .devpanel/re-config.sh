...
STATIC_FILES_PATH="$WEB_ROOT/drupal/web/sites/default/files/"
SETTINGS_FILES_PATH="$WEB_ROOT/drupal/web/sites/default/settings.php"
...
#== Composer install.
if [[ -f "$APP_ROOT/drupal/composer.json" ]]; then
  cd $APP_ROOT/drupal && composer install
fi
